      ...
      <main className="flex-grow container mx-auto px-4 py-8 mt-20 pt-10">
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="text-center space-y-4">
            <h1 className="text-4xl font-bold text-lime-700">Добрини</h1>
          ...
          <section className="pt-8">
            ...
            <Input placeholder="Търсене по заглавие..." value={titleSearchTerm} onChange={e => setTitleSearchTerm(e.target.value)} className="mb-6 bg-background max-w-md mx-auto block" />

            {filteredGoodDeeds.length === 0 ? <p className="text-center text-gray-600">Няма одобрени добри дела, които да отговарят на търсенето.</p> : <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
                {filteredGoodDeeds.map(deed => <div key={deed.id} className="border rounded-md p-4 bg-white shadow-sm hover:shadow-md transition-shadow">
